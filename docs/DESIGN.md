...
- **2026-07-02** — **V2.2 — editorial (light) redesign + a11y/honesty fixes.** A light-theme editorial pass over the UI with accessibility and honesty-framing improvements; no data or schema change.
- **2026-07-02** — **`D_max` re-locked to 4,000 ft** for the V2 *routed* distribution (median p50 3,011 ft → 61.8% in-range), superseding the V1 straight-line 2,000 ft lock (which stays in the record below as what was true then). See §13.
- **2026-07-01** — **V2 — routed connector distance.** `build/routing.py` computes real road-following distance (multi-source Dijkstra over the OSM street graph to the `primary` corridor) and bakes it into the **same** schema columns — `connector_distance_ft` (routed feet) + `connector_geometry` (routed polyline, avg ~11 vertices, fetched via `connectors.parquet`). A pure data swap: the only web change was drawing the baked routed polyline instead of reconstructing a straight line, so circuity becomes vestigial (default 1.0) and barrier crossings are recomputed against the routed path. Realizes the §3 V1/V2 contract; the cell layer (§14) inherits the routed distances for free. See §3.
...
- **2026-06-29** — Repo named `nearnet-pgh` (later `nearnet-pittsburgh-metro`, §12); MIT code license locked; data-licensing table added (OSM/ODbL flagged as the share-alike one to watch, independent of MIT code license).
